$(document).ready(function() {
	...
	function changeUrl(pageNumber){
		var newUrl = '/admin-category?page=' + pageNumber;
		history.pushState(null, '', newUrl);
	}
	//Lay tham so url hien tai
	...
	function changePage(pageNumber) {
		if (pageNumber === 1) {
			$("#pagePrevious").hide();
			$("#pageNumberPrevious").addClass('disabled');
			$("#pageNumberCurrent").text(pageNumber);
			$("#pageNext").text(pageNumber+1);
		}
		else{
			$("#pagePrevious").text(pageNumber-1);
			$("#pageNumberCurrent").text(pageNumber);
			$("#pageNext").text(pageNumber+1);
			$("#pagePrevious").show();
			$("#pageNumberPrevious").removeClass('disabled');
		}
	}
	
	$("#pageNumberPrevious").click(function() {
		var pageNumber = $("#pageNumberCurrent").text();
		getTableByPage(parseInt(pageNumber)-1);
		changePage(parseInt(pageNumber)-1);
		changeUrl(parseInt(pageNumber)-1);
	...
	function getTableByPage(pageNumber) {
		$.ajax({
			method: "GET",
			url: "http://localhost:8888/api/admin/category/getbypage/" + pageNumber,
			headers: {
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.success) {
					...
					$("#confirmModal").modal('show');
					console.log(getUrlParam);
					getTableByPage(parseInt(getUrlParam()));
				} else {
